...
function AddEditResult() {
  ...
  for (const module in modules) {
    children.push(
      <Option key={modules[module]._id}>{modules[module].module_code}</Option>
    );
  }
  ...
  const onSearch = async (values) => {
    ...
    students.forEach((student) => {
      if (
        student.modules.filter((module) => module.module === values.module_code)
          .length > 0
      ) {
        st.push(student);
      }
  ...
  const onFinish = async (values) => {
    ...
    if (id) {
      const res = await dispatch(
        updateResult({ id, status: false, ...result })
      );
      if (res.status === 200) {
  ...
  const handleAdd = (add) => {
    if (result) {
      result.students.forEach((student) => {
        add({
          regNumber: student.student.regNumber,
        ...
        <div>
          ...
          >
            ...
            <div style={{ width: "600px", margin: "auto" }}>
              ...
              <p>
                Found<b> {studentList.length}</b> students
              </p>
                                  ...
                                  rules={[
                                    {
                                      required: true,
                                      message:
                                        "Missing Student Registration Number",
                                    },
                                  ]}
                                >
                                  <Input
