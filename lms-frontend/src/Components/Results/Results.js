...
const Results = () => {
  ...
  const publishConfirm = async (e) => {
    ...
    const res = await dispatch(
      updateResult({ id: e.key, ...updatedResult, status: true })
    );
    if (res?.status === 200) {
  ...
  const unPublishConfirm = async (e) => {
    const updatedResult = result.filter((res) => res._id === e.key)[0];
    const res = await dispatch(
      updateResult({ id: e.key, ...updatedResult, status: false })
    );
    if (res?.status === 200) {
    ...
    if (searchModule || searchStatus) {
      ...
      function searchFun(module) {
        return Object.keys(this).every((key) =>
          module[key].module_code
            .toLowerCase()
            .includes(this[key].module_code.toLowerCase())
        );
      }
